...
GEMINI_API_KEY can come from either a web/.env file (create one, same format
as backend/.env.example) or an environment variable set before launch
(e.g. $env:GEMINI_API_KEY="..." in PowerShell). Either works now.
"""
...
load_dotenv()  # reads web/.env if present; does nothing if it doesn't exist
...
if not os.environ.get("GEMINI_API_KEY"):
    st.warning(
        ...
        "Create web/.env (copy backend/.env.example) or set the env var before launch.",
        icon="⚠️",
    )
...
st.divider()
st.caption("AI Trust Checker · Built by Sai Sindhu Rachabattuni")